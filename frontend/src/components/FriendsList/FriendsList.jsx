...
function FriendsList({ currFriend }) {
  ...
  useEffect(() => {
    const sortedFriends = Object.values(friends).sort((friend1, friend2) =>
      friend1.name > friend2.name ? 1 : friend1.name < friend2.name ? -1 : 0
    );
  ...
  return (
    <div className="fiends-list-container">
      <h4
        ...
        onClick={() => (smallScreen ? handleSectionClick("friends") : null)}
      >
        FRIENDS
      </h4>
      <div
        className="friends-list"
        hidden={smallScreen && selectedSection !== "friends"}
      >
        {orderedFriends.map((friend) => (
          <div
            key={friend.id}
            className={
              currFriend && currFriend?.name === friend?.name
                ? "on-friends-page"
                : "friends-list-ele"
            }
          >
            <span onClick={() => handleFriendClick(friend.id)}>
              {friend.name}
            </span>
            ...
            <OpenModalButton
              ...
              modalComponent={
                Object.values(expenses).find(
                  (expense) =>
                    expense.receiverId === friend.id ||
                    expense.payerId === friend.id
                ) ? (
