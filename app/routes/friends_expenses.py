...
def all_friends_expenses():
    ...
    """
    payer_friends_expenses = (
        FriendsExpense.query.filter_by(payer_id=current_user.id)
        .order_by(FriendsExpense.expense_date)
        .all()
    )
    ...
    receiver_friends_expenses = (
        FriendsExpense.query.filter_by(receiver_id=current_user.id)
        .order_by(FriendsExpense.expense_date)
        .all()
    )
...
@friends_expenses_routes.route(
    "/<int:friends_expense_id>/update", methods=["GET", "POST"]
)
@login_required
